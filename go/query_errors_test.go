...
func TestDiagnosticsQuery_ImportedDefinitionFileID(t *testing.T) {
	loader := NewInMemoryImportLoader()
	// Main file references imported rule
	loader.Add("main.peg", []byte(`
@import Expr from "./expr.peg"
G <- Expr
`))
	// Expr file with a recursive rule (just to generate a diagnostic)
	loader.Add("expr.peg", []byte(`
Expr <- Term ('+' Term)*
Term <- Factor ('*' Factor)*
Factor <- '(' Expr ')' / [0-9]+
`))
	...
	// Should have recursive-rule info from expr.peg
	foundExprDiag := false
	for _, d := range diagnostics {
		if d.Code == "recursive-rule" && strings.Contains(d.Message, "Expr") {
			foundExprDiag = true
			if d.FilePath != "expr.peg" {
				t.Errorf("expected Expr recursive diagnostic from expr.peg, got %s", d.FilePath)
			}
			// Check line number is valid (should be line 2 in expr.peg)
			if d.Location.Span.Start.Line < 1 {
				t.Errorf("expected valid line number for Expr, got %d", d.Location.Span.Start.Line)
			}
		}
	}
	...
	if !foundExprDiag {
		t.Logf("diagnostics: %v", diagnostics)
		t.Error("expected recursive-rule info for Expr")
	}
}
