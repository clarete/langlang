 ...
 */
export function createLanglang(
    raw: Raw = (globalThis as any).langlang,
): langlang {
    if (!raw) {
        throw new Error(
            "langlang WASM bindings not found. Did you initialize the WASM module?",
        );
    }
...
export default class langlang {
    ...
    matcherFromString(grammar: string, cfg?: Config): Matcher {
        const mid = unwrap<{ id: number }>(
            this.raw.matcherFromString(String(grammar), cfg ?? undefined),
        ).id;
...
const matcherFinalizer: FinalizationRegistry<{ raw: Raw; id: number }> | null =
    canFinalize
...
export class Matcher {
    ...
    match(input: string): { consumed: number; value: Value } {
        ...
        return unwrap<{ consumed: number; value: Value }>(
            this.raw.match(this.id, String(input)),
        );
    }
