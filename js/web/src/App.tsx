...
function App() {
  ...
  const onGrammarEditorMount = useCallback<NonNullable<EditorProps["onMount"]>>(
    (editor, monaco) => {
      ...
      (monaco as any).editor.registerEditorOpener({
        openCodeEditor: (
          ...
          if (nextPath !== activeGrammarPath) setActiveGrammarPath(nextPath);
    ...
    return (
      <>
        <BarRoot>
          ...
          {showSettings ? (
            <MatcherSettingsPanel value={settings} onChange={setSettings} />
          ) : null}
        </BarRoot>
        <SplitView
          initialRatio={0.65}
                      ...
                      headerRight={
                        inputs.length > 1 ? (
                          <select
                            ...
                            onChange={(e) => setActiveInputPath(e.target.value)}
                          >
                            {inputs.map((f) => (
